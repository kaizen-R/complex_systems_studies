...
# library(MBCbook) ## For imshow() -- NOT used anymore

p0 <- 0.6 ## Initial prevalence of trees on the map
map_size_l <- 500 ## Map size, a matrix to be, so squared

# view_speed <- 0.08 ## animation speed (smaller == faster)
...
saveHTML({
  for(t_step in 1:1000) {
    ...
    image(the_map, axes=FALSE, useRaster = TRUE, col = c("white", "green", "red", "black"))
